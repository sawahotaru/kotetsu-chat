...
LLM を使わない間、ollama のイメージ（**約7GB**）もモデル（約1.8GB）も**置かれない**。
...
GPU が無いなら **1〜3B 級に限る**（既定は `qwen2.5:3b`）。7B 以上は CPU では実用にならない。
ディスクの空きを確認してから増やすこと（1モデル 0.8〜2GB）。

> 既定を 3B にしてあるのは**口調を保てるから**。同じ人格プロンプトで8問試すと、
> 1B は口調が崩れ、一人称の規則を破って役を降りることがある。
> ⚠ そのぶん冷えていると遅い（4コアCPUで54秒）。`OLLAMA_KEEP_ALIVE=-1` で
> 常駐させて初めて実用になる（2.4GB）。RAM が乏しければ `gemma3:1b` に戻すこと。
